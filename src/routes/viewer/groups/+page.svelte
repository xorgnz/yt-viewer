...
<h1>Choose a Channel Group</h1>

{#if data.groups.length === 0}
    <p>No channel groups yet. Ask an admin to create some in Admin → Channel Groups.</p>
{:else}
    <div class="grid">
        {#each data.groups as g}
            <a class="group" href={`/viewer?${new URLSearchParams({ groupId: String(g.id), profile: profileKey }).toString()}`}>
                <div class="name">{g.name}</div>
                <div class="hint">View videos from this group</div>
            </a>
        {/each}
    </div>
{/if}
