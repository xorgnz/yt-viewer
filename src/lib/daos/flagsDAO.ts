...
export class PostgresFlagsDAO extends PostgresDAO
{
    ...
    ): Promise<void>
    {
        ...
        await this.run(
            `UPDATE video_flags SET ${sets.join(', ')}, updated_at = ((EXTRACT(EPOCH FROM NOW()) * 1000)::BIGINT) WHERE video_id = ? AND profile_id = ?`,
            params
        );
    }
