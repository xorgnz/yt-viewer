...
export class PostgresSourceChannelDAO extends PostgresDAO
{
    ...
    {
        return this.getRow(`SELECT id, youtube_id, title, description, thumbnail_url, published_at, last_refreshed_at FROM source_channels WHERE id = ?`, [id]);
    }

    async getByExternalId(external_id: string): Promise<SourceChannel | undefined>
    {
        return this.getRow(`SELECT id, youtube_id, title, description, thumbnail_url, published_at, last_refreshed_at FROM source_channels WHERE youtube_id = ?`, [external_id]);
    }

    async list(): Promise<SourceChannel[]>
    {
        return this.listRows(`SELECT id, youtube_id, title, description, thumbnail_url, published_at, last_refreshed_at FROM source_channels ORDER BY title`);
    }
    ...
    async listWithVideoStats(): Promise<SourceChannelWithVideoStats[]>
    {
        return this.listRows(`
            SELECT
                ...
                COALESCE(SUM(CASE WHEN vf_agg.watched = 1 THEN 1 ELSE 0 END), 0)::INTEGER AS watched_count,
                COALESCE(SUM(CASE WHEN vf_agg.favorite = 1 THEN 1 ELSE 0 END), 0)::INTEGER AS favorite_count,
                COALESCE(SUM(CASE WHEN vf_agg.ignored = 1 THEN 1 ELSE 0 END), 0)::INTEGER AS ignored_count
            FROM source_channels sc
            LEFT JOIN videos v ON v.channel_id = sc.id
            LEFT JOIN (
                SELECT
