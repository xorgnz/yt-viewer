...
- [x] 1.0 Replace the data model to support virtual-channel source associations with per-association mode and review state
...
- [x] 2.0 Add admin routes and server actions for managing a single virtual channel's source-channel associations
...
- [ ] 4.0 Implement filtering and temporary bulk-selection tools for selected-only video review
  ...
  - [x] 4.3 Add video-type filtering for long, short, and unknown classification where available
  - [x] 4.4 Add temporary bulk actions for the currently shown rows, including select-all-shown and select-none-shown behavior
...
- [ ] 6.0 Validate the new admin management flow with targeted automated tests
  ...
  - [x] 6.5 Run targeted validation for the new admin management flow without starting long-running processes
