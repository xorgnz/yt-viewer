# SQLite-to-MySQL/MariaDB Cutover

## Purpose

This document defines the manual cutover sequence for moving existing SQLite data into the MySQL/MariaDB runtime introduced by `08-online-deploy`.

## Prerequisites

- The source SQLite database file is present and readable.
- `DATABASE_URL` points at the intended MySQL/MariaDB target.
- The MySQL/MariaDB target is reachable and backed up according to the vendor's normal tooling.
...
2. Back up or snapshot the MySQL/MariaDB target if it contains any data.
3. Run the MySQL/MariaDB schema bootstrap:
...
- Every table count must show matching SQLite and MySQL/MariaDB counts.
...
8. Keep the SQLite backup until the MySQL/MariaDB-backed app has been validated.
...
3. Back up the SQLite source file and snapshot the target MySQL/MariaDB database.
4. Set `DATABASE_URL` to the production MySQL/MariaDB target from Secret Manager or the vendor console.
...
10. Keep the SQLite backup until MySQL/MariaDB runtime behavior is confirmed and rollback is no longer needed.
...
- Fix the cause, restore or reset the MySQL/MariaDB target from backup, and rerun the migration.
...
- Runtime traffic is confirmed on MySQL/MariaDB.
