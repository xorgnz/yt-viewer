...
## Overview
Deploy the yt-viewer SvelteKit application to Google Cloud (GCP) using Cloud Run. Ship a Node-based SvelteKit server build (adapter-node) and connect to an external Postgres database vendor. Manage secrets with Google Cloud Secret Manager. Keep environments minimal: local dev and prod only.
...
- Enable database connectivity to an external Postgres vendor (implementation timing clarified below).
...
- Wire database credentials for Postgres via Secret Manager:
  - Preferred single-secret option: `DATABASE_URL` (e.g., `postgres://user:pass@host:5432/db?sslmode=require`), or
  - Discrete vars: `PGHOST`, `PGPORT`, `PGDATABASE`, `PGUSER`, `PGPASSWORD` (and `PGSSLMODE` as needed).
...
- Data layer: Current code uses SQLite via `better-sqlite3`. Cloud Run file system is ephemeral; SQLite is not production-suitable on Cloud Run. Postgres integration is intended but may be tackled as a follow-up feature if not included here.
...
- Database migration: Implementing a Postgres client and DAO changes is significant; plan as separate tasks if not folded into this feature.
...
## Open Questions
1. Postgres integration timing: implement in this feature or defer as a follow-up feature after initial deploy validation?
2. Secret format preference: single `DATABASE_URL` vs. discrete `PG*` variables?
